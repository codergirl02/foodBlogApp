...
import com.example.androidkickoff.adaptor.CartItemAdapter
...
import com.example.androidkickoff.util.PLACE_ORDER
...
class CartActivity : AppCompatActivity() {
    ...
    private fun init() {
        ...
        txtResName.text = RestaurentsFragment.resName
    ...
    private fun setUpCartList() {
        ...
        * of food items*/
        for (element in dbList) {
            orderList.addAll(
                    Gson().fromJson(element.foodItems,
                            Array<FoodItem>::class.java).asList()
            )
        }
//
...
//        /*Else we display the cart using the cart item adapter*/
    ...
    private fun sendServerRequest() {
        ...
        jsonParams.put("restaurant_id", RestaurentsFragment.resId?.toString() as String)
        var sum = 0
        for (i in 0 until orderList.size) {
            sum += orderList[i].cost as Int
        }
