...
import com.example.androidkickoff.activity.CartActivity
import com.example.androidkickoff.adaptor.RestaurentMenuAdapter
import com.example.androidkickoff.database.FoodDatabase
import com.example.androidkickoff.database.OrderEntity
...
class RestaurentsFragment : Fragment() {

    private lateinit var recyclerMenu: RecyclerView
    private lateinit var restaurantMenuAdapter: RestaurentMenuAdapter
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_restaurents, container, false)
                            ...
                            try {
                                ...
                                if (success) {
                                    ...
                                    for (i in 0 until resArray.length()) {
                                        ...
                                        restaurantMenuAdapter = RestaurentMenuAdapter(
                                            activity as Context,
                                            menuList,
                                                object : RestaurentMenuAdapter.OnItemClickListener {
                                                override fun onAddItemClick(foodItem: FoodItem) {
                                                    orderList.add(foodItem)
                                                    if (orderList.size > 0) {
                                                        goToCart.visibility = View.VISIBLE
                                                        RestaurentMenuAdapter.isCartEmpty = false
                                                    }
                                                }
                                                ...
                                                override fun onRemoveItemClick(foodItem: FoodItem) {
                                                    ...
                                                    if (orderList.isEmpty()) {
                                                        goToCart.visibility = View.GONE
                                                        RestaurentMenuAdapter.isCartEmpty = true
                                                    }
                                                }
                                            })
